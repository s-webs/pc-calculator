...
function deleteComponent(id) {
    if (confirm('Вы уверены, что хотите удалить этот компонент?')) {
        router.delete(route('pc-components.destroy', id));
    }
}
</script>

<template>
    <AppLayout title="Компоненты ПК">
        <template #header>
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <div>
                        <Link :href="route('pc-components.create')" class="block px-[30px] py-[20px] bg-green-500 text-white font-semibold text-center">Добавить</Link>
                    </div>
                    <div>
                        <div v-for="(item, index) in items" class="bg-blue-200 mt-2 p-4 flex items-center justify-between flex-wrap">
                            <div class="flex items-center">
                                ...
                                <div class="mr-2">{{ item.name }} | Себестоимость: {{ item.prime_cost }} | Цена продажи: {{ item.sale_price}} | Количество: {{ item.quantity }} | В наличии: {{ item.available ? 'Да' : 'Нет' }}
                                </div>
                            </div>
                            <div class="flex items-center">
                                <Link :href="route('pc-components.edit', item.id)" class=" mr-2">
